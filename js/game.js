...
function selectPet(petId) {
    ...
    gameState.animationInterval = setInterval(() => {
        ...
        const currentFrame = petGame.innerHTML.includes(currentStage[0])
            ? 1
            : 0;
...
function updateMeters() {
    // Clamp values
    gameState.hunger = Math.max(
        0,
        Math.min(gameState.maxHunger, gameState.hunger)
    );
    gameState.happiness = Math.max(
        0,
        Math.min(gameState.maxHappiness, gameState.happiness)
    );

    // Update meter widths
    hungerMeter.style.width =
        (gameState.hunger / gameState.maxHunger) * 100 + "%";
    happinessMeter.style.width =
        (gameState.happiness / gameState.maxHappiness) * 100 + "%";

    // Update text values
    hungerValue.textContent =
        Math.round(gameState.hunger) + "/" + gameState.maxHunger;
    happinessValue.textContent =
        Math.round(gameState.happiness) + "/" + gameState.maxHappiness;
    ...
    setTimeout(() => {
        ...
        gameState.animationInterval = setInterval(() => {
            ...
            const currentFrame = petGame.innerHTML.includes(currentStage[0])
                ? 1
                : 0;
            petGame.innerHTML = `<img src="${
                currentStage[currentFrame]
            }" alt="${PETS[gameState.selectedPet].name}">`;
        }, 500);
...
function petDied() {
    ...
    petGame.innerHTML = `<img src="${deadImage}" alt="Dead ${
...
function setupEventListeners() {
    ...
    document
        .getElementById("keepPlayingBtn")
        .addEventListener("click", keepPlaying);
    document
